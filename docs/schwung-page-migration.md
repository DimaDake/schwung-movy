...
| SP-59 | Automated parameters on a delegated page: a lane wears its OWN mark, and a held step decorates only the cells it locks. **movy-side shipped on `feat/sp-59-automated-mark`; the mark needs SU-8 upstream** (charlesvestal/schwung#541). movy feature-detects `drawAutomatedMark`: against a library that has it the lane rides `io.isAutomated` (2x2 beside the label, no tilde, same `:base`/`:effective` motion); against an older one it keeps SP-36's `isModulated` fold, so **no Schwung floor change**. `decorationsFor` now drops a lane the held step does not lock — it was the bare top-left corner on every automated cell (the reported "dot on the top left"), and against #509's inverted band it would have shown the LIVE value as the step's. **Schwung's own lock corner is declined** (`exact: false` on every decoration, the user's call): movy's lane already wears the always-on 2x2 beside the label, a second dot says it twice, the corner is invisible on a cell a graphic covers, and movy has no curve for `exact` to distinguish from. Pre-#509 Schwung ignores the field and still draws it. The inverted lock value and graphics-stay-while-held are upstream's #509, already on `origin/main`; the device ran 1.4.0+SU-14 (`1959e661`), which has neither. Teeth: `logic/page-automation.mjs` SP-59 blocks (decoration filter and channel choice each red with the fix removed), `test_automated_mark.sh` upstream (mark, clamp gap and motion each red). New scenes `page_lane_mark`/`_held` go through the owner's own `automationFor` wiring; their baselines are the reference Schwung's (tilde) and change when it moves | Opus | 🔨 | — | SU-8 |
...
| SU-11 | A per-key duration in the animation store, so `settled` ages out a value that never rests | 🔨 **FILED — charlesvestal/schwung#543 (2026-09-26), RESHAPED.** Not "age a never-resting key out" as written above: that also freezes the final tween of a fast knob turn longer than one duration (the last detent lands, nothing asks for the rest). Shipped as `activity(state, now)` → `{ moving, streaming }` beside an unchanged `settled()`, each key judged by the duration `observe()` was given for it — the host redraws on `moving` and throttles on `streaming`. movy's SP-48 `repaint-cap.ts` can then ASK instead of inferring from its 500 ms grace window; the cap stays until a release carries #543 |
...
| SU-13 | A host-owned page's write throttle and knob feel | ⬜ **deliberately NOT filed with the #543 bundle (2026-09-26).** Still conditional and still no complaint; SP-57 H2 then showed the knob-rate half is host-routable (`rawPerDetent`), and SP-53's throttle scare was test realism, not a production bug. Open only on a real feel complaint about the throttle itself |
| SU-14 | The re-plan skip: `param_pages` re-plans the whole module even when the contract has not changed | ✅ **MERGED — schwung PR #519, 2026-09-17** (the "OPEN and unreviewed" status that stood here was stale; corrected 2026-09-26 from `gh pr list`). Reaches a device with the first release containing it; until then SP-49's residual stands |
...
| SU-16 | **NEW, SP-57.** A 2-option enum may declare `turn: "absolute"` — stepped by direction instead of toggled on every detent. The toggle stays the default and stays right (a boxed two-way shows a state, not a direction); it is wrong only where the pair is ORDERED and the caller knows it | 🔨 **FILED — charlesvestal/schwung#543 (2026-09-26)**, `turn: "absolute"` in `knob_engine.mjs`. Deletes SP-57 H3's host-side routing once a release carries it |
| SU-17 | **NEW, SP-57 — and NOT blocking: movy ships a host-side equivalent.** `ctl.dismissPeek()` is public, so the host can take the panel down on the turn that raised it (SP-57 H6); the upstream hook is still worth having so a module need not re-implement the dismissal, but nothing waits on it. `io.enumPeek(key, meta)` — a host may decline the option panel per key. Generalises the rule `page_controller.mjs` ALREADY applies to a list layout ("a list row already prints the option in full") to a grid cell whose box fits the whole option. Can only decline, never force | 🔨 **FILED — charlesvestal/schwung#543 (2026-09-26)**, named `io.allowEnumPeek(fullKey, meta)` rather than `io.enumPeek` (the controller already exports `enumPeek()` as the getter). Asked last, can only decline. movy's H6 `dismissPeek` stays until a release carries it |
| SU-18 | **NEW, SP-57 — and NOT blocking: movy ships a host-side widget meanwhile** (`custom:movy_big_value`, SP-57 H7), which is deleted when this lands. A param may declare `display: "big"` — the value is read, not aimed, whatever it looks like. Lifts the span cap AND the blanket enum refusal for declaring params only; draws the text the page already resolved instead of recomputing it from the raw number; replaces the three-digit guard with a measured width. Needs `font_big_num.mjs` to gain `:` `%` `/` `.` — transcribed from the same MIT source its header already names (movy's `src/font/glyphs-big.ts`, a full ASCII atlas this project vendored twelve glyphs of) | 🔨 **FILED — charlesvestal/schwung#543 (2026-09-26)**: `display: "big"` + a measured-width fit (widest text + glyphs present, else the old widget) + `: % / .` in the face. the face also gained the note names `A`–`G` `#`, so **all four movy big cells migrate** — step COND (widest `8:8` 24px), clip LENGTH (`256` 29px), set SWING (`80%` 30px via `formatValue`), set ROOT (`C#` 19px), of a 30px budget; `test_big_value_declared.sh` walks every value of all four. When a release carries #543, the four `viz: { kind: BIG_VALUE_KIND }` become `display: "big"` and `renderer/schwung-big-value.ts` is DELETED outright |
