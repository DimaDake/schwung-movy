...
# The three files seeded below are scp'd, and scp OPENS THE DESTINATION FOR
# WRITING — refused on a root-owned file however writable the directory is, the
# same hazard ts_seq_apply unlinks around. Movy has usually just saved them as
# root, so they go with the version store rather than being written over.
SEEDED="$D/seq-state.json $D/seq-state.1.json $D/ui-state.json"
ts_ssh "rm -rf $D/v $D/versions.json $SEEDED" 2>/dev/null \
    || ts_ssh_root "rm -rf $D/v $D/versions.json $SEEDED" \
    || { echo "test-versions: cannot clear $D — movy writes it as root and root ssh is unavailable" >&2; exit 1; }
