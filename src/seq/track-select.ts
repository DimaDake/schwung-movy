...
/* True while the Session button is physically down. Gates the selected-track
 * read-out: holding the button is a question ("where am I?"), so the answer
 * belongs to the hold. Latched Session view is somewhere you sit and work, and
 * a permanent white step there just competes with the group pulse. */
export function sessionButtonHeld(): boolean {
...
export function sessionStepLed(step: number, focusGroup: number, selectedTrack: number): CellLed {
     ...
     *
     * SOLID white, not a pulse. It only ever shows while the Session button is
     * held (the caller passes -1 otherwise), so it is a momentary read-out
     * rather than a permanent state, and stillness is what separates it from
     * everything around it — the neighbours are already pulsing. A pulse here
     * had to share the one animation channel with the group's, which left the
     * two either in antiphase or indistinguishable. */
    if (step === selectedTrack) return { base: C_WHITE, anim: C_WHITE, channel: ANIM_NONE };
