export interface ParamVM {
    ...
    options:         string[] | null;
    enumIndex:       number;
    renderStyle:     'arc' | 'hbar' | 'vbar' | 'pan' | 'switch' | 'preset' | 'xbox' | 'steps' | 'items' | 'wave' | 'envstage' | 'cut';   // xbox = framed X (LFO target None); switch = boolean on/off (hbar is the retired alias); pan = bipolar centre-out bar (stereo placement); steps = framed number (octave/voice count); items = framed item name (bank/soundfont selector); wave = waveform silhouette; envstage = a lone attack/decay ramp
     ...
     * from the bottom) for an ordinary level. */
